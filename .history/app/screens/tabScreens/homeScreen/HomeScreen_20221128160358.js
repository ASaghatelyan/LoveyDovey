    ...
    const onGetLoverMatch = async (arr1) => {
        ...
        try {
            ...
            const newData = _copyData.map(item => {
                if ((category?.length > 1 &&   res?.data?.data?.category_id ? category : res.data.data.category_id).includes(item.id)) {
                    item.status = true
                }
                else if (!(category?.length > 1 &&   res?.data?.data?.category_id ? category : res.data.data.category_id).includes(item.id)) {
                    item.status = false
                }
                return item
            })
            res?.data?.data?.category_id && setChooseData(newData)
            setLoad(false)
        } catch (e) {
            console.log(e, 'err2');
            setLoad(false)
