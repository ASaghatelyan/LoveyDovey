import {
    View, Text, ScrollView,
    TouchableOpacity, StatusBar,
    ImageBackground, Image,
    SafeAreaView, SafeAreaProvider,
} from 'react-native'
import React, { useState, useEffect } from 'react'
import { styles } from './style'
import { BgImage, LoadingModal } from 'app/components'
import { TextInput as PaperInput } from 'react-native-paper';

import bg from 'app/assets/img/white.png'
import back from 'app/assets/img/back.png'
import noData from 'app/assets/img/noData.png'
import loc from 'app/assets/img/loc.png'
import mess from 'app/assets/img/mess.png'
import { GlobalButton } from 'app/components/globalButton';
import axiosInstance from 'app/networking/api';
import moment from 'moment';
import del from 'app/assets/img/close.png'

export function Notification(props) {
    const [loading, setLoading] = useState(false)
    const [data, setData] = useState([])
    
    let requestFunc = async () => {
        try {
            setLoading(true)
            let res = await axiosInstance.get(`/user/notification`)
            setData(res.data.data)
            setLoading(false)
        } catch (e) {
            console.log(e, 'err');
            setLoading(false)
        }
    }

    useEffect(() => { 
        requestFunc()
    }, [])
    let delNotification = async (id) => {
        try {
            setLoading(true)
            let res = await axiosInstance.post(`/user/notification/delete?id=${id}`)
 
            requestFunc()
            setLoading(false)
        } catch (e) {
            console.log(e, 'err');
            setLoading(false)
        }
    }

    let delAllNotification = async () => {
        try {
            setLoading(true)
            let res = await axiosInstance.get(`/user/notification/delete/all`)
            console.log(res);   requestFunc()
            setLoading(false)
        } catch (e) {
            console.log(e, 'err');
            setLoading(false)
        }
    }

    return (
        <View style={{ flex: 1, height: '100%', backgroundColor: 'red' }}>
            <BgImage img={bg} />
            <SafeAreaView
                style={styles.mainContainer}>
                <StatusBar
                    animated={true}
                    barStyle='dark-content'
                    translucent={true}
                />
                <View style={styles.content}  >
                    <View style={styles.topTitle}>
                        <TouchableOpacity onPress={() => props.navigation.goBack()}>
                            <Image source={back} style={styles.back} />
                        </TouchableOpacity>
                        <View style={styles.gFlex}>
                            <Text style={styles.titleText}>Notifications</Text>
                        </View>
                    </View>
                </View>
                <ScrollView
                    showsVerticalScrollIndicator={false}
                    bounces={false}
                    contentContainerStyle={[styles.bottomView, data.length < 1 ? { justifyContent: 'center', alignItems: 'center' } : { backgroundColor: 'white' }]}>
                    {data.length < 1 ? <View style={styles.noDataView}>
                        <Image source={noData} style={styles.noData} />
                        <Text style={styles.noDataText}>No data found</Text>
                    </View> :
                        <View style={{ alignItems: 'flex-end' }}>
                            <TouchableOpacity onPress={() => delAllNotification()}>
                                <Text style={styles.deletAll}>Delete All</Text>
                            </TouchableOpacity>
                            {data?.map((item, index) => {
                                return <View key={item.id} style={styles.generalView}>
                                    <View style={styles.titleView}>
                                        <Text style={styles.title}>{moment(item.need_or_want.start).format('DD MMM, YYYY')} - {moment(item.need_or_want.end).format('DD MMM, YYYY')}</Text>
                                        <TouchableOpacity onPress={() => {
                                            delNotification(item.id)
                                        }}>
                                            <Image source={del} style={styles.delIc} />
                                        </TouchableOpacity>
                                    </View>
                                    <Text style={styles.description}>{item.need_or_want.description}</Text>
                                </View>
                            })}
                        </View>}
                </ScrollView>
            </SafeAreaView>
            <LoadingModal isVisible={loading} />
        </View>
    )
}