...
export function WantNeedInfo(props) {
    ...
    let onDone = async () => {
        try {
           let a= await axiosInstance.post(`/user/need-or-want/status/update`, {
                id: props.route.params.id,
                status: 'Done'
            })
                ...
                <ScrollView contentContainerStyle={styles.content}  >
                    ...
                    <ScrollView contentContainerStyle={[styles.content, { paddingHorizontal: 16 }]}  >
                        <View>
                            ...
                            />
                            {props.route.params.status !== 'Done' && <GlobalButton diffStyle={{ backgroundColor: '#C4C0BF' }} btnName="Mark as Missed" onSubmit={onMissed} />}
                            {props.route.params.status !== 'Done' && <GlobalButton btnName="Mark as Done" onSubmit={onDone} />}
                        </View>
                    </ScrollView>
